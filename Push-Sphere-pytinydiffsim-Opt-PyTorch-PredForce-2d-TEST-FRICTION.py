...
def push(force, friction, step, render, tensor, check_friction):
    ...
    for iter in range(step):
        ...
        #   SIMULATING A NON-CONSTANT SURFACE FRICTION  /   MODEL 1:
        """
        Obj_pos_X, Obj_pos_Y = sphere_body.world_pose.position[0], sphere_body.world_pose.position[1]
        CoF = ((1./100.) * abs(Obj_pos_X * Obj_pos_Y)) + (2./10.)
        """
        
        #   SIMULATING A NON-CONSTANT SURFACE FRICTION  /   MODEL 2:
        #"""
        min_CoF = .1
        max_CoF = .45
        CoF = rand(min_CoF, max_CoF)
        #"""
        ...
        if check_friction:
            if render:
                render_CoF_value = "Coefficient of friction = " + str(CoF)
                app.draw_text_3d(render_CoF_value, 0, 0, 8, 1)
            else:
                print("Object position, frame #", iter, ":", sphere_body.world_pose.position)
                print(" |   Friction = ", CoF)
...
def Optimization(friction, target, N_interactions_Opt, steps_sim, learning_rate):
    ...
    for i in range(N_interactions_Opt):
        ...
        _, simulated_target = push(predicted_force, friction, steps_sim, render=False, tensor=True, check_friction=False)
        simulated_target_Tensor = torch.tensor([simulated_target]).type(dtype)
        #print('\nsimulated_target_Tensor:', simulated_target_Tensor)
...
while 1:
    ...
    print(  "\n| Mass:", mass_sphere,
            ...
            "\n| Friction:", friction,
            "\n| Target:", target)
    push(force_opt, friction, steps, render, tensor=False, check_friction=True)
    sleep(5)